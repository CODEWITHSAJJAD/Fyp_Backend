...
def date_to_string_custom(date_obj):
    """Convert datetime object to Wed-D-M-YYYY string format"""
    if date_obj is None:
        return None
    if isinstance(date_obj, str):
        return date_obj
    # Format: %a = abbreviated weekday name (Mon, Tue, etc.)
    #         %-d = day without leading zero (1, 2, 3, etc.)
    #         %-m = month without leading zero (1, 2, 3, etc.)
    #         %Y = 4-digit year
...
def add_days_to_string(date_str, days):
    ...
    if date_obj is None:
        return None
    new_date = date_obj + timedelta(days=days)
    return date_to_string_custom(new_date)
