...
'OutOfScope': "I am specialized in agriculture in Pakistan. Please ask me about crops, soil, or farming!",
    ...
    def generate_response(self, query, farmer_id=None, session_id=None):
        ...
        try:
            ...
            if farmer_id:
                ...
                # Override for "it rained today should I give water" -> Irrigation
                ...
                # Override for "what to grow in next season rabi/kharif" -> CropRecommendation
                ...
                # Override for "activities performed on most profitable crop" -> PastActivities
                ...
                # Override for "loss crop" or "lost money" -> unprofitable sessions
                ...
                # Override for "remaining activities" or "pending activities" -> get from suggested activities
                ...
                # Override for "most profitable crop" -> single most profitable
            ...
            # REMOVED: Short query and low confidence OutOfScope check
    ...
    def _handle_context_response(self, query, farmer_id, context, session_id=None):
        ...
        # Determine the correct intent based on last_intent for handling the response
        last_intent = context.get('last_intent', 'CropRecommendation')
    ...
    def _handle_context_intent(self, query, intent, farmer_id, context, session_id=None, is_quick=False, is_explanatory=False):
        ...
        if intent == 'MyLands':
            ...
            # Check if asking about activities on most profitable crop
            has_most_profitable = 'most profitable' in query_lower or 'most profited' in query_lower
            ...
            # Check for loss crops activities
            has_loss = any(word in query_lower for word in ['loss', 'lost', 'negative', 'not profitable', 'unprofitable'])
            if has_loss:
                from Services.ChatbotHelper import get_activities_for_profitable_sessions
            ...
            if has_loss and has_activities_req:
                # Activities for loss crops
                all_sessions = get_profitable_sessions(farmer_id, limit=100)
                unprofitable = [s for s in all_sessions if s.get('is_profit') == 0]
                if not unprofitable:
                    return {'answer': "No loss crops found in your history.", 'intent': intent}
                response = f"Activities performed on crops that resulted in loss:\n"
                for i, s in enumerate(unprofitable[:3], 1):
                    response += f"\n{i}. {s['crop_name']} on {s['land_name']}:\n"
                    response += get_activities_for_profitable_sessions(farmer_id, s.get('land_name')) + "\n"
                return {'answer': response, 'intent': intent}
            
            if has_loss:
                # Show loss crops - get all and filter
                all_sessions = get_profitable_past_crops(farmer_id)
                return {'answer': all_sessions, 'intent': intent}
            ...
            from Services.ChatbotHelper import get_profitable_past_crops, get_neighbor_past_sessions, get_activities_for_profitable_sessions
            # Check if asking about neighbor's profitable crops
            ...
            # Check if asking for MOST/SINGLE most profitable crop
            ...
            if has_most_word and not has_activities_word:
                # Return only the single most profitable crop from get_profitable_past_crops
                response = get_profitable_past_crops(farmer_id)
            ...
            # Check if asking for activities on most profitable crop
            if has_activities_word and has_most_word:
                # Get activities for most profitable crop
                response = get_activities_for_profitable_sessions(farmer_id, None)
                return {'answer': response, 'intent': intent}
            
            # Default: show all profitable crops
            response = get_profitable_past_crops(farmer_id)
            return {'answer': response, 'intent': intent}
            ...
            # Check if there's an active session with this crop first
            from Services.ChatbotHelper import get_active_sessions_for_farmer, get_upcoming_activities
            active_sessions = get_active_sessions_for_farmer(farmer_id)
            ...
            if crop_name:
                for session in active_sessions:
                    if session.crop_name.lower() == crop_name.lower():
                        active_session = session
                        break
            
            if active_session:
                activities = get_upcoming_activities(farmer_id, getattr(active_session, 'cultivation_session_id', None))
                response = f"Harvest information for your active {crop_name} crop:\n"
                if activities and 'Harvest' in str(activities):
            ...
            if active_sessions and len(active_sessions) > 0:
                ...
                for session in active_sessions[:3]:
                    response += f"\n• {session.crop_name} on {session.land_name}: "
                    activities = get_upcoming_activities(farmer_id, getattr(session, 'cultivation_session_id', None))
                    if activities and 'Watering' in str(activities):
