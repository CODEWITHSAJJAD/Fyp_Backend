...
class ActivitySuggestionService:
    ...
    def seed_suggested_activities(session_id, sowing_date_str):
        try:
            session = CultivationSessionModel.query.filter(
                CultivationSessionModel.cultivation_session_id == session_id).first()
            if not session:
    ...
    def get_suggested_activities(session_id):
        try:
            ...
            if adjustments_made and farmer_id:
                notif_msg = f" Weather adjusted: " + ", ".join(
                    [f"{x['activity_name']} → {x['new_date']}" for x in adjustments_made])
    ...
    def record_performed(session_id,activity_id, performed_date_str):
        try:
            print(performed_date_str)
            sa = SuggestedActivityModel.query.filter(SuggestedActivityModel.cultivation_session_id==session_id,SuggestedActivityModel.suggested_date==performed_date_str).first()
            if not sa:
    ...
    def skip_activity(suggested_activity_id, reason):
        try:
            sa = SuggestedActivityModel.query.filter(
                SuggestedActivityModel.suggested_activity_id == suggested_activity_id).first()
            if not sa:
